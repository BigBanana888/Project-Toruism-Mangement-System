...
export default {
  ...
  mounted() {this.user = JSON.parse(this.$store.getStore('user'))
  ...
  methods: {
    async loadData() {
      try {
        this.isLoading = true
        this.searchKey.aid = this.user.id
    ...
    async markAsReplied(item) {
      this.$router.push({path:"/user-chat",query:{id:item.createdId}})
    },
